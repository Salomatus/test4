...
def filter_by_currency(transactions: Iterable[dict], currency_code: str)\
        -> Iterator[dict]:
    """Функция принимает список словарей (или объект-генератор) с банковскими
    операциями и возвращает итератор, выдает по очереди операции, в которых
    указаны заданные валюты."""
